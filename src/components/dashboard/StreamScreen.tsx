      ...
      <BackTopBar
        ...
        right={
          <div className="flex items-center gap-2">
            {isLiveNow ? (
              <span className="inline-flex items-center gap-[6px] text-[10px] font-bold text-[#EF4444] text-white">
                <i />
                LIVE
              </span>
            ) : null}
        ...
        <div className="px-4 pt-[14px] pb-[calc(86px+var(--eventz-safe-area-bottom))]">
          <div className="bg-white border border-[#E9EBF0] rounded-[18px] p-[17px] mb-[13px] mt-1">
            ...
            {isLiveNow ? (
              ...
              <div className="border border-dashed border-[#E5E7EB] bg-[#FAFAFA] rounded-xl py-[18px] px-[18px] text-center text-[#6B7280] text-[12px] font-semibold">No live stream is running right now.</div>
            )}
          ...
          <button type="button" className="w-full rounded-[14px] py-[14px] px-3 mt-[11px] flex items-center justify-center gap-[9px] text-[13px] font-bold tracking-[.02em] bg-gradient-to-br from-[#7C3AED] to-[#5B21B6] text-white disabled:opacity-50 disabled:grayscale-[.25] disabled:cursor-not-allowed" onClick={() => onGo('notify')} disabled={!isLiveNow}>
