          ...
          <div className="relative" ref={selectorRef}>
            ...
            {events && onEventChange ? (
              <button 
                ...
                className="flex items-center gap-1 text-xs text-gray-500 hover:text-primary transition-colors mt-0.5 group"
              >
                <span className="line-clamp-1 max-w-[200px] text-left">{eventTitle}</span>
                <ChevronDown className={`w-3 h-3 transition-transform ${showEventSelector ? 'rotate-180 text-primary' : ''}`} />
              </button>
            ) : (
                ...
                <div className="p-2 space-y-1">
                  {events.map((event) => (
                    ...
                    >
                      ...
                      {event.id === eventId && <CheckCircle2 className="w-4 h-4 text-primary" />}
                    </button>
                  ))}
                </div>
              </div>
