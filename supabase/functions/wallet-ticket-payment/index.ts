...
async function ntzsRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  if (!NTZS_API_KEY) throw new Error("Missing NTZS_API_KEY");
  ...
  const body = contentType.includes("application/json")
    ...
    : await response.text();
  if (!response.ok) {
    const err: Error & { status?: number; details?: unknown } = new Error(
...
async function getOrCreateNtzsUser(externalId: string, email: string) {
  return await ntzsRequest<{ id: string; balanceTzs?: number; balance?: number }>("/users", {
    method: "POST",
...
function generateIdempotencyKey(userId: string, eventId: string, amount: string): string {
  ...
  return `wallet-payment-${userId}-${eventId}-${amount}-${timestamp}`;
}
...
async function rollbackNtzsTransfer(transferId: string) {
  try {
    ...
    console.error("[wallet-ticket-payment] Rollback failed:", rollbackError);
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await setIdempotencyPending(admin, userId, idempotencyKey, "wallet-ticket-payment");
    ...
    if (eventError || !event?.organizer_id) {
      await setIdempotencyFailed(admin, userId, idempotencyKey, { message: "Event organizer not found" });
      return corsResponse({ error: "Event organizer not found" }, 404, req);
    }

    if (event.organizer_id === userId) {
      await setIdempotencyFailed(admin, userId, idempotencyKey, { message: "Cannot purchase own ticket" });
      return corsResponse({ error: "You cannot purchase your own ticket" }, 400, req);
    }
    ...
    if (balance < amount) {
      const errorMsg = `Insufficient wallet balance. You have TSh ${balance.toLocaleString()} but need TSh ${amount.toLocaleString()}.`;
    ...
    try {
      ...
      console.error("[wallet-ticket-payment] DB insert failed, rolling back transfer:", dbError);
      if (transfer.id) {
    ...
    const debit = insertedTransactions.find(
      (tx: any) => tx.user_id === userId && tx.metadata?.type === "payment"
    );

    const result = { transactionId: debit?.id, transfer };
    await setIdempotencyCompleted(admin, userId, idempotencyKey, result);

    return corsResponse(result, 200, req);
  } catch (err: any) {
    console.error("[wallet-ticket-payment] Error:", err);
