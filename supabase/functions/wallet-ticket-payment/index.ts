...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (eventError || !event?.organizer_id) {
      return jsonResponse({ error: "Event organizer not found" }, 404);
    }

    if (event.organizer_id === userId) {
      return jsonResponse({ error: "You cannot purchase your own ticket" }, 400);
    }
    ...
    if (balance < amount) {
      return jsonResponse(
        {
          error: `Insufficient wallet balance. You have TSh ${balance.toLocaleString()} but need TSh ${amount.toLocaleString()}.`,
        },
        400
      );
    }
    ...
    return jsonResponse({ transactionId: debit?.id, transfer });
  } catch (err: any) {
